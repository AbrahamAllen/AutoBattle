...
class popup{
	constructor(x, y, txt, r = false, parent = screen){
		...
		if(document.getElementsByClassName('popup')[0]){document.getElementsByClassName('popup')[0].remove()};
...
/* 		let baseLvl = this.hero.lvl+5;
		...
		new menu(screen, 'openStatsM', '', function(){u.hero.mainStatMenu()}); 
	...
	newFloor(){
		if(this.chest){u.openChest(u.activeTower.chests[u.floorTrack])};
		domclear(screen);
		this.floorTrack++;
	...
	floorMenu(){
		...
		try{this.activeTower.floorPreview(this.floorTrack)}catch{new popup(10, 10, 'BOSS DEFEATED')};
		
		new menu(main.dom, 'openBag', '', function(){u.bag.open()});
		new menu(main.dom, 'openStats', '', function(){u.hero.mainStatMenu()});
		new menu(main.dom, 'leaveTower', '', function(){u.endTower()});
		new menu(screen, 'nextFloor', '', function(){u.newFloor()});
		let div = new menu(main.dom, 'speedClimb', '', function(){u.speedClimbSwitch()});
		if(this.speedClimb){setTimeout(function(){if(u.speedClimb){u.newFloor()}}, 2000)};
	}
	...
	speedClimbSwitch(){
		let div = document.getElementById('speedClimb')
		if(this.speedClimb){this.speedClimb = false; div.style.backgroundColor = 'red'}else{this.speedClimb = true; div.style.backgroundColor = 'green'};
	}
...
class Tower{
	...
	floorPreview(i){
		new menu(document.getElementById('floorEnd'), 'floorNo');
		document.getElementById('floorNo').innerHTML = i+1;
		
		let preview = new menu(document.getElementById('floorEnd'), 'preview');
		
		for(let enemy of this.floors[i+1]){
			let previewEnemy = new menu(preview.dom, enemy.constructor.name, 'preview', function(){enemy.displayStats()});
			previewEnemy.dom.style.backgroundImage = enemy.img;
			previewEnemy.dom.innerHTML = enemy.lvl;
...
class Character{
	...
	setStats(){
		...
		this.maxhp = Math.ceil((this.tof)+(this.stm*2)+(this.str)+(this.lvl));
	}
...
class Hero extends Character{
	...
	changeStance(stance){
		...
		try{
			...
			let div = document.getElementById('statBottomBar');
			domclear(div);
			div.remove();
			this.statBottomBar();
		}
	...
	mainStatMenu(){
		...
		this.statBottomBar();
		this.statTopBar();
		let main = new menu(screen, 'statHolder');
		
		let stats = Object.keys(this);
		let statVal = Object.values(this);
		
		for(let i = 0; i<3; i++){
			let div = new menu(main.dom, stats[i], 'stat', function(){u.hero.usePoint(this.id); u.hero.mainStatMenu()});
			div.dom.innerHTML = statVal[i];
			div.dom.style.backgroundImage = "url('css/assets/"+stats[i]+".png')";
		}
		document.getElementById('stm').style.marginLeft = '120px';
		
		new menu(screen, 'swap', 'swap', function(){u.hero.advancedStatMenu()});
		new menu(screen, 'back', 'back', function(){if(u.activeTower){u.floorMenu()}else{u.mainmenu()}});
	...
	advancedStatMenu(){
		...
		for(let i = 3; i<7; i++){
			let div = new menu(main.dom, stats[i], 'stat', function(){u.hero.usePoint(this.id); u.hero.advancedStatMenu()});
			div.dom.innerHTML = statVal[i];
			div.dom.style.backgroundImage = "url('css/assets/"+stats[i]+".png')";
		}
		...
		new menu(screen, 'swap', 'swap', function(){u.hero.mainStatMenu()});
		new menu(screen, 'back', 'back', function(){if(u.activeTower){u.floorMenu()}else{u.mainmenu()}});
	...
	statTopBar(){
		...
		new menu(screen, 'expBar');
		let expTrack = new menu(document.getElementById('expBar'), 'expTrack');
		let n = this.exp/(this.lvl*15);
		n*=100; 
		expTrack.dom.style.width = n.toString()+'%';
		
		let main = new menu(screen, 'statTopBar');
		for(let i = 9; i<12; i++){
			let div = new menu(main.dom, stats[i], 'barStat', function(){u.hero.usePoint(this.id); u.hero.mainStatMenu()});
			div.dom.innerHTML = statVal[i];
			div.dom.style.backgroundImage = "url('css/assets/"+stats[i]+".png')";
		}
		document.getElementById('exp').style.width = '170px';
		document.getElementById('exp').innerHTML += '/ '+this.lvl*15;
	...
	statBottomBar(){
		let stats = Object.keys(this);
		let statVal = Object.values(this);
		let main = new menu(screen, 'statBottomBar');
		for(let i = 12; i<16; i++){
			let div = new menu(main.dom, stats[i], 'barStat', function(){u.hero.usePoint(this.id); u.hero.mainStatMenu()});
			div.dom.innerHTML = statVal[i];
			div.dom.style.backgroundImage = "url('css/assets/"+stats[i]+".png')";
		}
		new menu(main.dom, 'stance', '', function(){u.hero.swapStance()});
		document.getElementById('stance').innerHTML = this.stance;
		
		document.getElementById('maxhp').innerHTML = this.hp+'/'+this.maxhp;
	}
	...
	statUp(stat, amt){
		switch(stat){
			...
			case 'stm' : this.stm += amt; this.hp+=amt; break;
...
class Enemy extends Character{
	...
	displayStats(){
		let statString = this.constructor.name+'<br>';
...
class Bag{
	...
	showGearBonus(){
		try{document.getElementById('gearBonus').remove()}catch{};
		new menu(document.getElementById('charMenu'), 'gearBonus');
		document.getElementById('gearBonus').innerHTML = '<br>' + u.hero.gatk + '<br><br>' + u.hero.gdef;
	}
	...
	forge(){
		...
		new menu(main.dom, 'inspectGear', '', function(){
			...
			if(div.childNodes[1]){u.bag.cancelForge()};
			u.bag.material = undefined;
	...
	populateGearSell(){
		...
		for(let gear of Object.values(this.gear)){
			...
			item.dom.style.backgroundImage = gear.img;
			item.dom.innerHTML = gear.lvl+':'+gear.rank+ '<br>' + gear.lvl*gear.rank*gear.stage;
			item.build();
		}
		new menu(screen, 'back', 'back', function(){if(u.activeTower){u.floorMenu()}else{u.mainmenu()}});
	}
	...
	merge(item){
		try{
			if(this.inspecting && this.inspecting.constructor.name == item.constructor.name){
			...
			} else if(this.inspecting.constructor.name == 'Drop' && item.gear){
					document.getElementById('inspectGear').appendChild(document.getElementById(item.id));
					document.getElementById(item.id).style.marginTop = '50px';
					this.material = item;
					document.getElementById('attachmentMenu').remove();
					new menu(document.getElementById('inspectMenu'), 'forge', '', function(){u.bag.enhance('Drop')});
					
					return true;
			
			}else if(this.inspecting.constructor.name == 'Ring' && item.gear){
					document.getElementById('inspectGear').appendChild(document.getElementById(item.id));
					document.getElementById(item.id).style.marginTop = '50px';
					this.material = item;
					document.getElementById('attachmentMenu').remove();
					new menu(document.getElementById('inspectMenu'), 'forge', '', function(){u.bag.enhance('Ring')});
	...
	enhance(type){
		...
		if(type == 'Drop'){this.add(new GearList[this.material.constructor.name](this.material.lvl+this.inspecting.bonus, this.material.rank, this.material.stage))};
		if(type == 'Ring'){this.add(new GearList[this.material.constructor.name](this.material.lvl, this.material.rank+this.inspecting.bonus, this.material.stage))};
		
		this.material.destroy(); this.material = undefined; this.inspecting.destroy(); this.inspecting = undefined; 
		this.forge();
	}
	...
	cancelForge(){
		...
		this.material = undefined; this.inspecting = undefined;
...
let GearList = {'Helm': Helm, 'Armor': Armor,'Legs': Legs,'Sword': Sword, 'Shield': Shield,'Spear': Spear,'Dagger': Dagger,'Axe': Axe, 'Ring': Ring};
...
class Drop{
	...
	attach(){
		if(u.bag.inspecting){
			if(u.bag.material){u.bag.cancelForge(); this.inspect(); return};
			if(u.bag.merge(this)){return};
			let div = document.getElementById(this.id);
...
function newGame(){
	...
	shop = new Shop();
	u.bag.add(new Armor(3, 2));
	u.bag.add(new Sword(2, 2));
	
	u.bag.add(new Drop('str', 1));
	
	u.bag.add(new Ring(5, 2));
