...
class Hero extends Character{
	...
	statMenu(){
		...
		for(let i = 0; i < 6; i++){
			...
			baseStat.dom.oncontextmenu = function(){event.preventDefault(); u.hero.allPoints(this.id)};
			baseStat.build();
		}
...
function newGame(){
	...
	u.bag.add(new Sword(Math.ceil(Math.random()*4), 2));
	u.bag.add(new Helm(1, 2));
	u.bag.add(new Helm(1, 2));
	u.bag.add(new Sword(1, 2));
	u.bag.add(new Sword(1, 2));
...
function loadGame(){
	...
	for(let drop of drops){
		let list = drop.split(',');
		u.bag.add(new Drop(list[0], list[1], list[2]));
	}
...
function saveGame(){
	...
	for(let i = 0; i < 6; i++){
		let string = BaseStats[i]+','+stats[i]+':';
		saveData += string;
	}
